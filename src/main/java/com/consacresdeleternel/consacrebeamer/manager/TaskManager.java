...
/**
 * TaskManger class provides a simple ThreadPool implementation to provide a Singleton resolution to exclude specific
 * work from the UI-Thread. The main goal is to increase Thread performance and to avoid the fact of creation constantly
 * new threads within the code. I decided to create this class after doing some research and performance tests. Also i
 * want to simplify the creation of task. The fact i had to write always new Thread(Runnable).start() and so on, annoyed
 * me...
 *
 * @author c.watzl
 */
...
public final class  TaskManager {
    ...
    private TaskManager() {
        ...
        EventHandler<WorkerStateEvent> taskHandler = evt -> {
            if (evt.getEventType().equals(WorkerStateEvent.WORKER_STATE_SUCCEEDED) ||
                ...
                LOG.info("WorkerStateEvent received for " + evt.getSource() + " EventType: " + evt.getEventType().getName());
        ...
        EventHandler<WorkerStateEvent> serviceHandler = evt -> {
            if (evt.getEventType().equals(WorkerStateEvent.WORKER_STATE_SUCCEEDED) ||
                ...
                LOG.info("WorkerStateEvent received for " + evt.getSource() + " EventType: " + evt.getEventType().getName());
                ((Service<?>) evt.getSource()).exceptionProperty().removeListener((observable, oldValue, newValue) -> exceptionHandler(newValue));
    ...
    /**
     * Method configureService configures a given service an prepares it and executes it
     *
     * @param service of type Service<?> the service to configure
     * @param verbose of type boolean lets the service run without progress binding
     */
     ...
     * Method configureService configures a given service an prepares it and executes it
     *
     * @param task    of type Task<?> the service to configure
     * @param verbose of type boolean lets the service run without progress binding
     */
